...
abstract class model
{
    ...
    public function __construct($select = false, $mod = false)
    {
        // объект бд коннекта
        global $dbObject;
        $dbObject = new \PDO('mysql:host=localhost;dbname=blogmvc;charset=utf8', 'root', '14678211');
        $this->db = $dbObject;

        // имя таблицы

        $class_name = get_class($this);
        $modelName = end(explode('\\', $class_name));
        ...
        if ($sql) {
            $this->_getResult('SELECT * FROM '.$mname.$sql);
        }
    }
